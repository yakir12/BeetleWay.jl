using BeetleWay
using Base.Test
using DataStructures, Base.Dates
...
@testset "util" begin 
    ...
    @test BeetleWay.second2hms(Second(59)) == Dict(Hour => 0, Minute => 0, Second => 59)
    @test BeetleWay.second2hms(Second(60 + 1)) == Dict(Hour => 0, Minute => 1, Second => 1)
    @test BeetleWay.second2hms(Second(60*60 + 1)) == Dict(Hour => 1, Minute => 0, Second => 1)
    @test BeetleWay.second2hms(Second(60*60*60)) == Dict(Hour => 60, Minute => 0, Second => 0)

    @test length(BeetleWay.shorten("1234567", 3)) == 7
    @test length(BeetleWay.shorten("1234567", 2)) == 5

    @test BeetleWay.findshortfile("aaa", Dict("aaa" => "b", "c" => "aaa")) == "c"
    @test_throws ErrorException BeetleWay.findshortfile("aaa", Dict("aaa" => "b", "c" => "aa")) == "c"

    t = (1977, 06, 01, 12, 0, 1)
    d1 = BeetleWay.validargs(:a, t...)
    @test isnull(d1)

    t = (1977, 02, 31, 12, 0, 1)
    d2 = BeetleWay.validargs(:a, t...)
    @test !isnull(d2)
...
#=@testset "GUIs" begin
    ...
    @test_nowarn BeetleWay.poi_gui(POI("a", Point("1",0), Point("2",0), "", ""), ["a", "b"], Dict(string(i) => string(i) for i in 1:2), "a")
